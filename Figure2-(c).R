...
result <- foreach(ii = 1:100)%dopar%{
  ...
  lambda.seq3=exp(seq(log(0.11),log(0.012),length.out=50))
  system.time(t3.path <- gnclassopath(X=X.std,A=Adj,lambda.seq=lambda.seq3,alpha=alpha.cv))
  t3.path$num.edge

 lambda.seq2=exp(seq(log(0.1),log(0.01),length.out=50))
  system.time(t2.path <- gnclassopath(X=X.std,A=Adj,lambda.seq=lambda.seq2,alpha=0.021*n/d))
  t2.path$num.edge
 
   lambda.seq1=exp(seq(log(0.16),log(0.041),length.out=50))
  system.time(t1.path <- rncglassopath(X=X.std,A=Adj,lambdaseq=lambda.seq1,alpha=0.07*n/log(n),safe=FALSE,unequal.scale = FALSE,verbose=TRUE,max.iter=20))
  t1.path$num.edge
...
save(result,file="GridSimulationResults-HighDim_LowSNR_CompareOracle-050.Rda")
