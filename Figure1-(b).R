...
X <- sim.data$data[1:n,]#%*%diag(sqrt(seq(1,5,length.out=100)))
...
result <- foreach(ii = 1:100)%dopar%{
  ...
  lambda.seq=exp(seq(log(0.29),log(0.95),length.out=50))
  ...
  lambda.seq3=exp(seq(log(0.25),log(0.05),length.out=50))
  system.time(t3.path <- gnclassopath(X=X.std,A=Adj,lambda.seq=lambda.seq3,alpha=alpha.cv))
  t3.path$num.edge
  lambda.seq1=exp(seq(log(0.3),log(0.051),length.out=50))
  system.time(t1.path <- gnclassopath(X=X.std,A=Adj,lambda.seq=lambda.seq1,alpha=0.065*n/d))
  t1.path$num.edge

  t2.path <- glassopath(new.S,rholist=exp(seq(log(0.26),log(0.066),length.out=50)))
  ...
  lambda.seq4=exp(seq(log(0.25),log(0.051),length.out=50))
  system.time(t4.path <- gnclassopath(X=X.std,A=Adj,lambda.seq=lambda.seq4,alpha=alpha.cv))
  t4.path$num.edge
   
  t0.err <- t1.err <- t2.err <- t3.err <- t4.err <- list()
  t0.graph <- t1.graph <- t2.graph <- t3.graph <- t4.graph <- list()
  ...
  dev.off()
  for(k in 1:length(t4.path$models)){
    A4 <- matrix(0,p,p)
    A4[abs(as.matrix(t4.path$models[[k]]$Theta))>1e-7] <- 1
    diag(A3) <- 0
    t4.graph[[k]] <- A4
    t4.err[[k]] <- max(abs(t4.path$models[[k]]$M-scaled.M))
  }
  png("tmp.png")
  t4.roc <- huge.roc(t4.graph, sim.data$theta, verbose = TRUE)
  dev.off()
  ...
  glasso.fp <- t2.roc$fp
  glasso.tp <- t2.roc$tp
  tmp <- list(gnc3.fp=gnc3.fp,gnc3.tp=gnc3.tp,glasso.fp=glasso.fp,glasso.tp=glasso.tp,gnc1.fp=gnc1.fp,gnc1.tp=gnc1.tp,gnc4.fp=gnc4.fp,gnc4.tp=gnc4.tp)
}
# 
...
save(result,file="GridSimulationResults-HighDim_LowSNR_NoIterative-005-GCV.Rda")
