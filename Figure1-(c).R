...
result <- foreach(ii = 1:100)%dopar%{
  ...
  lambda.seq3=exp(seq(log(0.14),log(0.028),length.out=50))
  system.time(t3.path <- gnclassopath(X=X.std,A=Adj,lambda.seq=lambda.seq3,alpha=alpha.cv))
  t3.path$num.edge
  lambda.seq1=exp(seq(log(0.16),log(0.03),length.out=50))
  system.time(t1.path <- gnclassopath(X=X.std,A=Adj,lambda.seq=lambda.seq1,alpha=0.021*n/d))
  t1.path$num.edge
 
  t2.path <- glassopath(new.S,rholist=exp(seq(log(0.5),log(0.18),length.out=50)))
  ...
  lambda.seq4=exp(seq(log(0.16),log(0.038),length.out=50))
...
save(result,file="GridSimulationResults-HighDim_LowSNR_NoIterative-050-GCV.Rda")
